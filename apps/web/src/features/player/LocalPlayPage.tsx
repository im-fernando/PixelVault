...
export function LocalPlayPage({ id }: Props) {
  ...
  if (isPending) {
    return <div className="aspect-video w-full animate-pulse rounded-xl bg-vault-800" />;
  }

  if (rom === null || fonte === null) {
    return (
      <div className="rounded-xl border border-vault-800 bg-vault-900 p-8 text-center">
        <h2 className="font-semibold text-accent">ROM local não encontrada</h2>
        <p className="mx-auto mt-2 max-w-md text-sm text-vault-300">
