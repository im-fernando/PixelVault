 ...
 * chega na M5. A #88 trouxe a tabela e a porta de leitura; a #89 trouxe a
 * gravação (`POST /progress/sram/:romId`, condicionada por revisão do ADR
 * 0020); a #90 trouxe a leitura (`GET /progress/sram/:romId`). Ele não é
 * dono da sessão (pede ao `sessions`), da autorização (pergunta ao
 * `identity`), do storage (porta compartilhada da aplicação) nem da ROM em
 * si (pergunta ao `library` se aquele `romId` é de quem está pedindo, pela
 * fachada dele).
 */
